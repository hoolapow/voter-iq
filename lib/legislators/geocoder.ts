...
export interface GeoResult {
  ...
  district: number | null  // congressional district number
}

// Static fallback: first 3 zip digits → state abbreviation
// Covers most common zip prefixes for graceful degradation
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  '005': 'MA', '010': 'MA', '011': 'MA', '012': 'MA', '013': 'MA', '014': 'MA',
  '015': 'MA', '016': 'MA', '017': 'MA', '018': 'MA', '019': 'MA', '020': 'MA',
  '021': 'MA', '022': 'MA', '023': 'MA', '024': 'MA', '025': 'MA', '026': 'MA',
  '027': 'MA', '028': 'RI', '029': 'RI',
  ...
  '728': 'AR', '729': 'AR',
  '730': 'OK', '731': 'OK', '733': 'OK', '734': 'OK', '735': 'OK', '736': 'OK',
  ...
  '876': 'NM', '877': 'NM', '878': 'NM', '879': 'NM', '880': 'NM', '881': 'NM',
...
export function zipToStateFallback(zipcode: string): string | null {
...
export async function geocodeZip(zipcode: string): Promise<GeoResult | null> {
  try {
    // Step 1: lat/lon
    const locUrl = `${CENSUS_BASE}/locations/address?zip=${encodeURIComponent(zipcode)}&benchmark=2020&format=json`
    const locRes = await fetch(locUrl, { signal: AbortSignal.timeout(5000) })
    if (!locRes.ok) throw new Error(`Census geocoder HTTP ${locRes.status}`)

    const locData = await locRes.json()
    const match = locData?.result?.addressMatches?.[0]
    if (!match) throw new Error('No address match from Census geocoder')

    const lat = match.coordinates.y as number
    const lon = match.coordinates.x as number

    // Step 2: congressional district from geography layer
    const geoUrl =
      `${CENSUS_BASE}/geographies/coordinates?x=${lon}&y=${lat}` +
      `&benchmark=Current_Current&vintage=Current_Current&layers=Congressional+Districts&format=json`
    const geoRes = await fetch(geoUrl, { signal: AbortSignal.timeout(5000) })

    let district: number | null = null
    let state = zipToStateFallback(zipcode) ?? 'US'

    if (geoRes.ok) {
      const geoData = await geoRes.json()
      const districts = geoData?.result?.geographies?.['Congressional Districts']
      if (districts?.length) {
        const cd = districts[0]
        const geoid: string = cd.GEOID ?? ''
        // GEOID = 2-digit state FIPS + 2-digit district number (e.g. "4821" = TX-21)
        if (geoid.length >= 4) {
          district = parseInt(geoid.slice(2), 10) || null
        }
        // Map state FIPS to abbreviation
        const stateAbbr = FIPS_TO_STATE[geoid.slice(0, 2)]
    ...
    // Graceful fallback: return null to trigger mock/senators-only mode
    return null
  }
}
